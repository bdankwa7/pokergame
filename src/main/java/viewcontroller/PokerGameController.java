...
public class PokerGameController {
     ...
     * @param  amount The amount of chips you want to start with
     */
    public PokerGameController(PokerGame gameView, String name, int amount) {

        this.gameView = gameView;

        human = new HumanPlayer(name, amount);
        ais = new AIPlayer[3];
        players = new Player[ais.length + 1];
        players[0] = human;
        for (int i = 0; i < ais.length; i++) {
            ais[i] = new AIPlayer("AI_" + (i + 1), amount);
            players[i + 1] = ais[i];
        }
